...
async def fetch_waterfall_failed_tasks(
    ...
    # Expect pre-normalized variants list from the tool layer.
    variants = arguments.get("variants") or []
    if not variants:
        raise ValueError(
            "At least one variant must be provided via the variants parameter"
        )

    statuses = arguments.get("statuses") or DEFAULT_FAILED_STATUSES
    waterfall_limit = arguments.get("waterfall_limit", 200)
